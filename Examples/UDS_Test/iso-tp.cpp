...
void IsoTp::print_buffer(INT32U id, uint8_t *buffer, uint16_t len)
{
  ...
  Serial1.print(F("Buffer: "));
  Serial1.print(id,HEX); Serial1.print(F(" ["));
  Serial1.print(len); Serial1.print(F("] "));
  for(i=0;i<len;i++)
  {
    if(buffer[i] < 0x10) Serial1.print(F("0"));
...
{
#ifdef ISO_TP_DEBUG
  Serial1.println(F("Send CAN RAW Data:"));
...
#ifdef ISO_TP_DEBUG
     Serial1.println(F("Received CAN RAW Data:"));
...
#ifdef ISO_TP_DEBUG
  Serial1.print(F("First frame received with message length: "));
  Serial1.println(rest);
  Serial1.println(F("Send flow controll."));
  Serial1.print(F("ISO-TP state: ")); Serial1.println(msg->tp_state);
#endif
...
uint8_t IsoTp::rcv_cf(struct Message_t* msg)
{
  ...
  {
#ifdef ISO_TP_DEBUG
    Serial1.println(F("CF frame timeout during receive wait_cf="));
    Serial1.print(wait_cf); Serial1.print(F(" delta="));
...
#ifdef ISO_TP_DEBUG
  Serial1.print(F("ISO-TP state: ")); Serial1.println(msg->tp_state);
  Serial1.print(F("CF received with message rest length: "));
  Serial1.println(rest);
#endif
  ...
  {
#ifdef ISO_TP_DEBUG
    Serial1.print(F("Got sequence ID: ")); Serial1.print(rxBuffer[0] & 0x0F);
    Serial1.print(F(" Expected: ")); Serial1.println(msg->seq_id & 0x0F);
#endif
    msg->tp_state = ISOTP_IDLE;
    msg->seq_id = 1;
    return 1;
...
#ifdef ISO_TP_DEBUG
    Serial1.print(F("Last CF received with seq. ID: "));
    Serial1.println(msg->seq_id);
#endif
  }
  else
  {
#ifdef ISO_TP_DEBUG
    Serial1.print(F("CF received with seq. ID: "));
    Serial1.println(msg->seq_id);
#endif
...
#ifdef ISO_TP_DEBUG
  Serial1.print(F("FC frame: FS "));
  Serial1.print(rxBuffer[0]&0x0F);
  Serial1.print(F(", Blocksize "));
  Serial1.print(msg->blocksize);
  Serial1.print(F(", Min. separation Time "));
  Serial1.println(msg->min_sep_time);
#endif
  ...
  switch (rxBuffer[0] & 0x0F)
  {
    ...
    case ISOTP_FC_WT:
			 ...
			 if(fc_wait_frames >= MAX_FCWAIT_FRAME)
                         {
#ifdef ISO_TP_DEBUG
                           Serial1.println(F("FC wait frames exceeded."));
#endif
                           fc_wait_frames=0;
                           msg->tp_state = ISOTP_IDLE;
                           retval=1;
...
#ifdef ISO_TP_DEBUG
                         Serial1.println(F("Start waiting for next FC"));
#endif
                         break;
    case ISOTP_FC_OVFLW:
#ifdef ISO_TP_DEBUG
                         Serial1.println(F("Overflow in receiver side"));
#endif
    default:
                         msg->tp_state = ISOTP_IDLE;
                         retval=1;
...
uint8_t IsoTp::send(Message_t* msg)
{
  ...
  while(msg->tp_state!=ISOTP_IDLE && msg->tp_state!=ISOTP_ERROR)
  {
...
#ifdef ISO_TP_DEBUG
    Serial1.print(F("ISO-TP State: ")); Serial1.println(msg->tp_state);
    Serial1.print(F("Length      : ")); Serial1.println(msg->len);
#endif
    ...
    switch(msg->tp_state)
    {
...
#ifdef ISO_TP_DEBUG
                                 Serial1.println(F("Wait first FC"));
#endif
                                 delta=millis()-wait_fc;
                                 if(delta >= TIMEOUT_FC)
                                 {
#ifdef ISO_TP_DEBUG
                                   Serial1.print(F("FC timeout during receive"));
                                   Serial1.print(F(" wait_fc="));
                                   Serial1.print(wait_fc);
                                   Serial1.print(F(" delta="));
                                   Serial1.println(delta);
#endif
                                   msg->tp_state = ISOTP_IDLE;
				   retval=1;
      ...
      case ISOTP_WAIT_FC      :
#ifdef ISO_TP_DEBUG
                                 Serial1.println(F("Wait FC"));
#endif
                                 break;
      case ISOTP_SEND_CF      :
#ifdef ISO_TP_DEBUG
                                 Serial1.println(F("Send CF"));
#endif
                                 while(msg->len>7 & !bs)
                                 {
                                   ...
                                   {
#ifdef ISO_TP_DEBUG
                                     Serial1.print(F("Send Seq "));
                                     Serial1.println(msg->seq_id);
#endif
                                     if(msg->blocksize > 0)
                                     {
#ifdef ISO_TP_DEBUG
                                       Serial1.print(F("Blocksize trigger "));
                                       Serial1.print(msg->seq_id %
                                                    msg->blocksize);
#endif
...
uint8_t IsoTp::receive(Message_t* msg)
{
  ...
  wait_session=millis();
#ifdef ISO_TP_DEBUG
  Serial1.println(F("Start receive..."));
#endif
  msg->tp_state=ISOTP_IDLE;
  ...
  while(msg->tp_state!=ISOTP_FINISHED && msg->tp_state!=ISOTP_ERROR)
  {
    ...
    {
#ifdef ISO_TP_DEBUG
      Serial1.print(F("ISO-TP Session timeout wait_session="));
      Serial1.print(wait_session); Serial1.print(F(" delta="));
      Serial1.println(delta);
#endif
      return 1;
    }
...
#ifdef ISO_TP_DEBUG
  Serial1.println(F("ISO-TP message received:"));
